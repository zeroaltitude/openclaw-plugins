...
ENDPOINT_CASES: list[tuple[str, dict, str]] = [
    ...
    ("/ingest", {"content": "c", "node_type": "fact", "tags": ["t"], "context": "ctx"}, "smart_ingest"),
    (
        "/smart_ingest",
...
def test_importance_score_forwards_context_topics_as_camel_case(client):
    ...
    # The bridge carries agent identity by prepending `agent:<id>` to topics.
    assert args["contextTopics"] == ["agent:tank", "x"]
