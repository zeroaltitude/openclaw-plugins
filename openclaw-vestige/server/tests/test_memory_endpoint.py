...
from app.models import MEMORY_ACTION_TO_ENGINE, MemoryAction
...
def test_memory_agent_id_does_not_displace_id(client):
    """X-Agent-Id adds context alongside `id` rather than replacing it."""
...
def test_every_bridge_action_is_known_to_the_engine():
    ...
    fails here unless MEMORY_ACTION_TO_ENGINE translates it.
    """
    for action in MemoryAction:
        engine_action = MEMORY_ACTION_TO_ENGINE.get(action, action.value)
